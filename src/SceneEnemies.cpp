...
void SceneEnemies::update(float dtime, SDL_Event *event)
{
	...
	case SDL_MOUSEBUTTONDOWN:
		...
		if (event->button.button == SDL_BUTTON_RIGHT) {
			...
			if (maze->isValidCell(cell))
			{
				enemiesPositionsID.push_back(GetNodeID(cell, agents[0]->getGraph()->w));
				agents[0]->addEnemyCost(enemiesPositionsID[enemiesPositionsID.size()-1], maze);
			}
		}
		break;
	default:
...
void SceneEnemies::draw()
{
	...
	agents[0]->draw();
	for each (int enemyPosID in enemiesPositionsID)
	{
		draw_circle(TheApp::Instance()->getRenderer(), (int)(maze->cell2pix(GetNodeCoords(enemyPosID, agents[0]->getGraph()->w)).x),
			(int)(maze->cell2pix(GetNodeCoords(enemyPosID, agents[0]->getGraph()->w)).y), 15, 255, 0, 0, 255);
	}
}
