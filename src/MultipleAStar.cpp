...
Path MultipleAStar::calculatePath(int _initialNodeID, int _finalNodeID, Graph * graph, Grid* grid)
{
	...
	if (earlyExit) // Si hem trobat el node final
	{
		...
		tempPath.totalCost = costSoFar[_finalNodeID];
...
Path MultipleAStar::calculateMultiplePath(int _initialNodeID, int _finalNodeID, std::vector<int> _vID, Graph * graph, Grid * grid)
{
	std::vector<Path> auxPath;
	Path finalPath;

	std::sort(_vID.begin(), _vID.end());
	...
	do {
		...
		if (_vID.size() > 0)
		{
			...
			Path p = calculatePath(_initialNodeID, _vID[0], graph, grid);
			auxPath.push_back(p);
			for (size_t i = 0; i < _vID.size() - 1; i++)
			{
				p = calculatePath(_vID[i], _vID[i + 1], graph, grid);
				auxPath.push_back(p);
			}
			p = calculatePath(_vID[_vID.size() - 1], _finalNodeID, graph, grid);
			auxPath.push_back(p);

		}
		else
		{
			Path p = calculatePath(_initialNodeID, _finalNodeID, graph, grid);
			auxPath.push_back(p);
		}

		Path auxPath2;
		//Path auxPath2 = auxPath[0];
		//auxPath.erase(auxPath.begin());
		for (size_t i = 0; i < auxPath.size(); i++)
		{
			//std::cout << auxPath[i].points.size() << std::endl;
			for each (Vector2D pos in auxPath[i].points)
			{
				auxPath2.points.push_back(pos);
			}
			auxPath2.totalCost += auxPath[i].totalCost;
		}

		if (finalPath.totalCost == 0 || finalPath.totalCost > auxPath2.totalCost)
		{
			finalPath = auxPath2;
			
		}

	} while (std::next_permutation(_vID.begin(), _vID.end()));

	return finalPath;
}

float MultipleAStar::calculateHeuristic(int _currentNodeID, int _finalNodeID, int w)
{
